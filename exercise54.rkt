...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname exercise54) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ())))
(require 2htdp/image)
...
(define (fly x)
  (cond
    ...
    [(>= x YDELTA) (- x YDELTA)]
    [else (+ (image-height ROCKET) HEIGHT)]))
(define (main s)
  (big-bang s
            [to-draw show]
            [on-key launch]
            [on-tick fly 1]
            [stop-when (lambda(x)(and(real? x)(> x HEIGHT)))]))